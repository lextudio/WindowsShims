...
public sealed partial class MainPage : Page
{
    ...
    static string Snapshot(MainPage page)
    {
        ...
        var paragraphBorderRectCount = renderScope is null
            ? null
            : GetInternalProperty(renderScope, "ParagraphBorderRectCount")?.ToString();
        var cellVisualRectCount = renderScope is null
            ? null
            : GetInternalProperty(renderScope, "CellVisualRectCount")?.ToString();
        var cellBoxLayout = renderScope is null
            ? null
            : GetInternalProperty(renderScope, "CellBoxLayout")?.ToString();
        var cellBoxHeightLayout = renderScope is null
            ? null
            : GetInternalProperty(renderScope, "CellBoxHeightLayout")?.ToString();
        var lineYLayout = renderScope is null
            ? null
            : GetInternalProperty(renderScope, "LineYLayout")?.ToString();
        var fillBoxRectCount = renderScope is null
            ? null
            : GetInternalProperty(renderScope, "FillBoxRectCount")?.ToString();
        var inlineBackgroundRectCount = renderScope is null
            ? null
            : GetInternalProperty(renderScope, "InlineBackgroundRectCount")?.ToString();
        ...
        var firstInlineImageRendered = FindFirstRenderedImageSource(firstInline);
    ...
    static bool FindFirstRenderedImageSource(WpfInline? inline)
    {
        while (inline is not null)
        {
