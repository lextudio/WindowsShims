...
public sealed partial class MainPage
{
    ...
    [
        ...
        new("sorting", "Sorting",
            "Multi-column sorting with sort-direction glyphs on column headers.",
            DataGridScenarios.BuildSortingGrid),
        new("selection", "Selection Modes",
            "Extended selection mode with CellOrRowHeader selection unit.",
            DataGridScenarios.BuildSelectionGrid),
        new("reorder", "Column Reorder",
            "Drag-to-reorder columns with CanUserReorderColumns enabled.",
            DataGridScenarios.BuildReorderGrid),
        new("clipboard", "Clipboard Copy",
            "ClipboardCopyMode with header-included copy of selected cells.",
            DataGridScenarios.BuildClipboardGrid),
        new("gridlines", "Grid Lines",
            "Horizontal-only grid lines with a custom accent-colored brush.",
            DataGridScenarios.BuildGridLinesGrid),
        new("headers", "Headers Visibility",
            "Column headers only (no row headers) via HeadersVisibility.Column.",
            DataGridScenarios.BuildHeadersGrid),
        new("column-types", "Column Types",
            "Text, CheckBox, ComboBox, and Hyperlink column types in one grid.",
            DataGridScenarios.BuildColumnTypesGrid),
        new("column-sizing", "Column Sizing",
            "Fixed, SizeToHeader, Star, and 2* column-width modes.",
            DataGridScenarios.BuildColumnSizingGrid),
        new("alternating-row", "Alternating Row",
            "AlternatingRowBackground with a subtle blue tint on every other row.",
            DataGridScenarios.BuildAlternatingRowGrid),
        new("large-data", "Large Data (10K)",
            "10,000 virtualized rows exercising row and column virtualization.",
