...
Integration tests: 238/238 (RichTextBox), 62/62 (DataGrid, drag-reorder test gated behind DATAGRID_DRAG_TESTS=1 pending host Accessibility permission).
...
- **RTF clipboard serialization** — WPF's `XamlRtfConverter` stack is linked;
  ...
  and nested lists (both levels, all text) round-trip intact. Session 89 added
  ...
  Session 92 added table column widths: `TableColumn.Width` round-trips
  ...
  parsed them with no changes.   Session 95 added coverage for `LineBreak`
  (`\line`) and tab characters (`\tab`), both already round-tripping.
  Session 96 added embedded images: under `HAS_UNO` the intermediate XAML
  carries self-contained `Source="data:image/png;base64,..."` data URIs —
  `WriteEmbeddedObject` emits them (encoded via SkiaSharp), the RTF writer
